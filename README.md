...
- A compact agent progress stream at `state/runtime-queue/progress-events.jsonl` with Telegram/Discord progress-panel delivery receipts and edit-in-place status updates.
...
Agent progress streaming is a separate compact status lane. Runtime pipeline writes `todo`, `skill_view`, `terminal`, `assistant_stream`, and terminal `run` events to `state/runtime-queue/progress-events.jsonl`. `progress-delivery-loop` renders the latest events for each queue item into one compact Telegram/Discord panel, edits the existing provider message when possible, rate-limits updates, truncates previews, redacts obvious secret-bearing previews, and writes `state/channels/progress-delivery-receipts.jsonl`. The bottom status line stays synchronized, for example `⏳ Working — 9 min — receiving stream response`, then becomes `✅ Done — ...` or `⚠️ Failed — ...`.
