...
const DETERMINISTIC_CRON_PLAN_SCHEMA: &str = "openclaw-harness.deterministic-cron-plan.v1";
...
pub fn load_deterministic_cron_store(
    source: &OpenClawSource,
) -> io::Result<DeterministicCronStore> {
...
mod tests {
    ...
    fn write_deterministic_source(root: &Path) -> OpenClawSource {
...
"#,
        ...
        OpenClawSource::with_workspace(root.join(".openclaw"), workspace)
    }
    ...
    fn temp_root(test_name: &str) -> PathBuf {
        ...
        std::env::temp_dir().join(format!(
            "openclaw-harness-deterministic-cron-{test_name}-{}-{nanos}",
            std::process::id()
        ))
    }
}
