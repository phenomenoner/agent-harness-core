...
const MEMORY_PROMPT_CONTEXT_RECEIPT_SCHEMA: &str =
    "openclaw-harness.memory-prompt-context-receipt.v1";
const MEMORY_LIFECYCLE_RECEIPT_SCHEMA: &str = "openclaw-harness.memory-lifecycle-receipt.v1";
const MEMORY_VECTOR_RECALL_RECEIPT_SCHEMA: &str =
    "openclaw-harness.memory-vector-recall-receipt.v1";
const MEMORY_CREDENTIALS_RECEIPT_SCHEMA: &str = "openclaw-harness.memory-credentials-receipt.v1";
const MEMORY_CANVAS_RECEIPT_SCHEMA: &str = "openclaw-harness.memory-canvas-receipt.v1";
...
const MEMORY_EMBEDDING_MODEL_ENV: &str = "OPENCLAW_HARNESS_MEMORY_EMBEDDING_MODEL";
const MEMORY_EMBEDDING_BASE_URL_ENV: &str = "OPENCLAW_HARNESS_MEMORY_EMBEDDING_BASE_URL";
...
pub fn search_imported_vector_memory(
    ...
    let Some(api_key) = embedding.api_key.as_deref() else {
        return Ok(MemoryVectorRecallReport {
            ...
            query_embedding_dim: 0,
            sqlite_database: Some(openclaw_mem_sqlite_file(&options.harness_home)),
...
pub fn search_imported_vector_memory_with_embedding(
    ...
    let sqlite = openclaw_mem_sqlite_file(harness_home);
    if !sqlite.is_file() {
        return Ok(MemoryVectorRecallReport {
            schema: MEMORY_VECTOR_RECALL_RECEIPT_SCHEMA,
    ...
    let capture_candidates_file = if !capture_candidates.is_empty() {
        ...
        for candidate in &capture_candidates {
            append_json_line(
                &file,
                &serde_json::json!({
                    "schema": "openclaw-harness.memory-auto-capture-candidate.v1",
...
pub fn run_memory_canvas_worker(
    ...
    let canvas = serde_json::json!({
        "schema": "openclaw-harness.symbolic-canvas.v1",
...
fn openclaw_mem_sqlite_file(harness_home: &Path) -> PathBuf {
