...
use crate::{AgentRegistry, OpenClawSource};

const NATIVE_CRON_PLAN_SCHEMA: &str = "openclaw-harness.native-cron-plan.v1";
...
pub fn load_native_cron_store(source: &OpenClawSource) -> io::Result<NativeCronStore> {
...
mod tests {
    ...
    fn write_cron_source(root: &Path) -> OpenClawSource {
    ...
    fn temp_root(test_name: &str) -> PathBuf {
        ...
        std::env::temp_dir().join(format!(
            "openclaw-harness-cron-{test_name}-{}-{nanos}",
            std::process::id()
        ))
    }
}
