...
const HARNESS_LOG_SCHEMA: &str = "openclaw-harness.log-event.v1";
...
pub fn probe_harness_log_writable(harness_home: impl AsRef<Path>) -> io::Result<PathBuf> {
    ...
    fs::create_dir_all(parent)?;
    let probe = parent.join(".openclaw-harness-log-probe.tmp");
...
mod tests {
    ...
    fn append_harness_log_writes_jsonl() {
        ...
        assert!(text.contains("\"schema\":\"openclaw-harness.log-event.v1\""));
    ...
    fn temp_root(test_name: &str) -> PathBuf {
        ...
        std::env::temp_dir().join(format!(
            "openclaw-harness-logging-{test_name}-{}-{nanos}",
            std::process::id()
        ))
    }
}
