...
const CHANNEL_OUTBOX_PLAN_SCHEMA: &str = "openclaw-harness.channel-outbox-plan.v1";
const CHANNEL_DELIVERY_RECEIPT_SCHEMA: &str = "openclaw-harness.channel-delivery-receipt.v1";
...
mod tests {
    ...
    fn temp_root(test_name: &str) -> PathBuf {
        ...
        std::env::temp_dir().join(format!(
            "openclaw-harness-channel-delivery-{test_name}-{}-{nanos}",
            std::process::id()
        ))
    }
}
