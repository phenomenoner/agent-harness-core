...
use crate::{
    ...
    ChannelReceiveStatus, HarnessLogEvent, HarnessLogLevel, OpenClawSource, PromptAssemblyOptions,
    RuntimeRunOnceOptions, RuntimeRunOnceReport, append_harness_log, build_runtime_skill_index,
    current_log_time_ms, plan_channel_outbox, receive_channel_message, run_runtime_queue_once,
};

const CHANNEL_RUN_ONCE_SCHEMA: &str = "openclaw-harness.channel-run-once.v1";
...
mod tests {
    use super::*;
    use crate::{
        ChannelOutboundMessageKind, ChannelRunOnceStatus, OpenClawSource, RuntimeRunOnceStatus,
    };
...
"#,
    ...
    fn write_channel_pipeline_source(root: &Path) -> OpenClawSource {
    ...
    fn temp_root(test_name: &str) -> PathBuf {
        ...
        std::env::temp_dir().join(format!(
            "openclaw-harness-channel-pipeline-{test_name}-{}-{nanos}",
            std::process::id()
        ))
    }
}
