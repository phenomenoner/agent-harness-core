...
const HARNESS_REGISTRY_SCHEMA: &str = "agent-harness.target-registry.v1";
const HARNESS_REGISTRY_RECEIPTS_SCHEMA: &str = "agent-harness.target-registry-receipts.v1";
...
mod tests {
    ...
    fn temp_root(test_name: &str) -> PathBuf {
        ...
        std::env::temp_dir().join(format!(
            "agent-harness-target-registry-{test_name}-{}-{nanos}",
            std::process::id()
        ))
    }
}
