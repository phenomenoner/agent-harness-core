...
const SUBAGENT_PLAN_SCHEMA: &str = "agent-harness.subagent-plan.v1";
...
pub fn load_subagent_ledger(source: &AgentSource) -> io::Result<SubagentLedger> {
...
mod tests {
    ...
    fn load_ledger_parses_keyed_object_runs() {
        ...
        let ledger =
            load_subagent_ledger(&AgentSource::with_workspace(&home, home.join("workspace")))
                .unwrap();
    ...
    fn temp_root(test_name: &str) -> PathBuf {
        ...
        std::env::temp_dir().join(format!(
            "agent-harness-subagents-{test_name}-{}-{nanos}",
            std::process::id()
        ))
    }
}
