...
use crate::{AgentSource, PROMPT_FILE_NAMES, SKILL_FILE_NAME};

const REPORT_SCHEMA: &str = "agent-harness.import-report.v1";
...
pub fn execute_import(options: ExecuteImportOptions) -> io::Result<ImportExecuteReport> {
    ...
    let report = ImportExecuteReport {
        schema: "agent-harness.import-execute-report.v1",
...
fn build_semantics(source: &AgentSource) -> ImportSemantics {
...
fn read_config_semantics(source: &AgentSource, semantics: &mut ImportSemantics) {
...
fn read_session_semantics(source: &AgentSource, semantics: &mut ImportSemantics) {
...
fn read_native_cron_semantics(source: &AgentSource, semantics: &mut ImportSemantics) {
...
fn agent_id_for_path(source: &AgentSource, path: &Path) -> Option<String> {
    let agents_root = source.home.join("agents");
...
fn render_summary_markdown(report: &ImportReport) -> String {
    ...
    out.push_str("# Agent Source Import Dry Run\n\n");
    out.push_str(&format!(
        "- Source home: `{}`\n",
...
mod tests {
    ...
    fn dry_run_report_plans_core_openclaw_paths() {
        ...
        let report = build_dry_run_report(DryRunImportOptions {
            source: AgentSource::new(&source_home),
            destination_home,
    ...
    fn write_report_files_outputs_json_and_markdown() {
        ...
        assert!(
            fs::read_to_string(files.summary)
                .unwrap()
                .contains("Agent Source Import Dry Run")
        );
    ...
    fn temp_root(test_name: &str) -> PathBuf {
        ...
        std::env::temp_dir().join(format!(
            "agent-harness-importer-{test_name}-{}-{nanos}",
            std::process::id()
        ))
    }
}
