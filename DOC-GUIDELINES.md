...
| [docs/agent-harness-operations-handbook.md](docs/agent-harness-operations-handbook.md) | Operational source of truth: live topology, current validation, full command walkthrough, capability ledger. | Operator and agent working sessions. |
...
| `docs/*.md` | One topic per file: tech notes, plans, handoffs, references. | Operator and agents. |

## Pick The Right Home

- A public claim, positioning statement, or onboarding step belongs in `README.md`.
- Live state, topology, command behavior, and operational procedure belong in the operations handbook.
...
- A behavior change updates the relevant handbook/tech-note section and adds a `CHANGELOG.md` entry in the same commit.
- A new document gets added to both documentation maps: the README "Documentation" table (if public-relevant) and the operations handbook documentation map.
