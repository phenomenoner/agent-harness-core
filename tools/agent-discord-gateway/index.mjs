...
function parseArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i += 1) {
    ...
    } else if (flag === "--source-home") {
      i += 1;
      args.sourceHome = requiredValue(argv, i, flag);
...
function defaultHarnessCli() {
  const exe = os.platform() === "win32" ? "agent-harness.exe" : "agent-harness";
...
function readAllowedDiscordUserIds(args) {
  const values = [];
  for (const key of ["AGENT_HARNESS_DISCORD_ALLOWED_USER_IDS", "DISCORD_ALLOWED_USER_IDS"]) {
    if (process.env[key]) {
...
function writeTempEventFile(payload) {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "agent-discord-event-"));
