...
| `agent-harness.inbound-media-artifact.v1` | `media` | Artifact metadata is additive in v1; prompt-facing consumers must treat raw payload/provider fields as redaction candidates and use bounded extraction summaries for durable context. | Round10 adds optional `lifecycleStatus` and `extractionSummary` fields so images, audio/transcripts, generated media, browser captures, documents/downloads, large tool logs/review transcripts, worker reports, and provider-native attachments can be represented by refs plus summaries instead of raw blobs. |
