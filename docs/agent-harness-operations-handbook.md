...
Supported modes are `off`, `progress_panel`, and `inline_preface`. The default `progress_panel` stores narration for audit, sends compact commentary to the editable progress status, keeps channel `agent-reply` text final-answer only, and records memory lifecycle from the final assistant text rather than narration. When commentary is not available yet, the progress status still renders a current-step fallback from the latest runtime/tool event.
