...
- Live process set: one bounded-concurrency `runtime-loop` plus `worker-loop`, `progress-delivery-loop`, `telegram-loop`, `discord-outbox-loop`, and `discord-gateway-loop`; `cron-scheduler-loop` is present only after an explicit scheduler cutover.
...
- Cron scheduler: `cron-scheduler-run-once`/`cron-scheduler-loop` evaluate imported native and deterministic cron sources, write watermarks under `state/cron-scheduler/watermarks.sqlite`, append scheduler receipts, and enqueue due worker jobs. Worker loops remain the only executors.
...
- A supervised runtime queue loop that drains queued work with bounded in-process concurrency, idle exit, bounded consecutive-error handling, exclusive Windows runtime lease locking, retry-pending lease cleanup, terminal failure/cancel statuses, and a `loop-last.json` status file.
...
- A cron scheduler tick/loop that reads imported cron registries and deterministic crontabs, writes durable scheduler watermarks/receipts, and idempotently enqueues due worker jobs without executing them.
...
- New channel/scheduler operations add `channel-identity-check`, `cron-scheduler-run-once`, and `cron-scheduler-loop` to that CLI surface.
