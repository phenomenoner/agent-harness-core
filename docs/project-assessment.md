...
- `plugin-sidecar-probe` starts a dependency-free Node sidecar probe, reads the imported harness registry and plugin install manifest, and writes `state/plugin-sidecar/probe.json` plus JSONL receipts. `plugin-sidecar-call` verifies the JSON-RPC bridge with `sidecar.status`, `plugins.list`, and `tools.list`, writing bridge receipts for `enable-check`. Actual OpenClaw hook/tool execution bridge is still pending.
...
- Long-running runtime worker loops, SQLite-consistent backup, Docker volume export, Windows Credential Manager vault migration, live channel bot handoff, scheduler execution, and plugin execution are still pending.
...
- Extend the runtime worker from one-shot `runtime-run-once` into a long-running worker loop: consume queued prompt bundles, start/resume sessions, stream events, update delivery outboxes, and persist transcript/trajectory/Codex binding receipts.
