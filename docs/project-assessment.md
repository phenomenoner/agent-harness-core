...
- `plugin-sidecar-probe` starts a dependency-free Node sidecar probe, reads the imported harness registry and plugin install manifest, writes `state/plugin-sidecar/probe.json` plus JSONL receipts, and exposes a minimal JSON-RPC contract for status/plugin/tool listing. Actual OpenClaw hook/tool execution bridge is still pending.
